...
# ----------------------------------------------------------------
# 背景執行緒：加入 Redis 去重鎖 (防止 LINE 重複推播)
# ----------------------------------------------------------------
...
def ticket_view(ticket_id):
    ...
    # 2. 身分驗證 (Session Guard)
    # 邏輯：如果使用者的瀏覽器有 Session，且 Session 裡的 ticket_id 與網址的不符 -> 禁止訪問
    # (這樣可以防止網頁端使用者亂改網址偷看別人)
    # (注意：LINE 內建瀏覽器通常沒有 Session，所以會跳過此檢查，這符合「LINE 用戶點連結即可看」的需求)
